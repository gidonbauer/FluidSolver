...
constexpr Index NX              = NY + NY / 2;
constexpr Index NGHOST          = 1;

constexpr Float X_MIN           = 0.0;
constexpr Float X_MAX           = 2.0 * std::numbers::pi;
constexpr Float Y_MIN           = -2.0;
...
constexpr auto vof0 = [](Float x, Float y) { return static_cast<Float>(y < std::sin(x)); };
...
auto main() -> int {
  ...
  while (t < T_END) {
    ...
    // Periodic boundary for volume fraction (left & right)
    for_each_a(fs.ym, [&](Index j) {
      vof.vf(-1, j) = vof.vf(NX - 1, j);
      vof.vf(NX, j) = vof.vf(0, j);
    });
    // Neumann boundary for volume fraction (top & bottom)
    for_each_i(fs.xm, [&](Index i) {
      vof.vf(i, -1) = vof.vf(i, 0);
      vof.vf(i, NY) = vof.vf(i, NY - 1);
    });
    calc_visc(vof.vf, fs);
