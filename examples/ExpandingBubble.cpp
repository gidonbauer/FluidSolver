...
constexpr Circle INIT_BUBBLE{.x = 0.0, .y = 0.0, .r = 0.25};

constexpr Float M_DOT           = 0.01;
...
auto main() -> int {
  ...
  Field2D<Float, NX, NY, NGHOST> m_dot_times_A{};
  Field2D<Float, NX, NY, NGHOST> avg{};
  ...
  for_each_a<Exec::Parallel>(vof.vf, [&](Index i, Index j) {
    vof.vf(i, j) =
        quadrature([](Float x, Float y) -> Float { return INIT_BUBBLE.contains({x, y}); },
                   fs.x(i),
                   fs.x(i + 1),
                   fs.y(j),
                   fs.y(j + 1)) /
  ...
  while (t < T_END) {
    ...
    // Shift the PLIC planes to account for phase change
    calc_interface_length(fs, vof);
    for_each_a<Exec::Parallel>(m_dot_times_A, [&](Index i, Index j) {
      if (!has_interface(vof.vf_old, i, j)) {
        m_dot_times_A(i, j) = 0.0;
        return;
      }
      m_dot_times_A(i, j) = vof.interface_length(i, j) * M_DOT;
    });

    for_each_i<Exec::Parallel>(vof.ir.interface, [&](Index i, Index j) {
      if (!has_interface(vof.vf_old, i, j)) { return; }
      auto& interface = vof.ir.interface(i, j);
      IGOR_ASSERT(interface.getNumberOfPlanes() == 1,
                  "Expected 1 plane but got {}",
                  interface.getNumberOfPlanes());
      IGOR_ASSERT(std::abs(fs.dx - fs.dy) < 1e-8, "dx != dy");
      auto& plane               = interface[0];
      const auto distance_corr  = (M_DOT * dt) / (RHO_L * fs.dx);
      plane.distance()         -= distance_corr;
    });
    ...
    for (Index sub_iter = 0; sub_iter < NUM_SUBITER; ++sub_iter) {
      ...
      for_each_i<Exec::Parallel>(avg, [&](Index i, Index j) {
        avg(i, j) = 0.0;
        for (Index ii = -1; ii <= 1; ++ii) {
          for (Index jj = -1; jj <= 1; ++jj) {
            avg(i, j) += vof.vf_old(i + ii, j + jj) >= VF_HIGH;
          }
        }
        avg(i, j) /= 9.0;
      });
      ...
      for_each_i<Exec::Parallel>(div, [&](Index i, Index j) {
        if (vof.vf_old(i, j) < VF_HIGH) { return; }
        Float m_dot_prime = 0.0;
        for (Index ii = -1; ii <= 1; ++ii) {
          for (Index jj = -1; jj <= 1; ++jj) {
            m_dot_prime += m_dot_times_A(i + ii, j + jj) / avg(i, j);
          }
        }
        div(i, j) -= m_dot_prime * (1.0 / RHO_G - 1.0 / RHO_L) * 1.0 / (fs.dx * fs.dy);
      });
