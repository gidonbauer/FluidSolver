...
constexpr Float Ca_L = VISC_L * U_IN_AVG / SURFACE_TENSION;
// Gas Capillary number
constexpr Float Ca_G = VISC_G * U_IN_AVG / SURFACE_TENSION;
// Ohnesorge number
constexpr Float Oh = Igor::sqrt(We) / Re_L;
...
auto main() -> int {
  ...
  Igor::Info("Capillary number (liquid) = {:.6e}", Ca_L);
  Igor::Info("Capillary number (gas)    = {:.6e}", Ca_G);
  Igor::Info("Reynolds number (liquid)  = {:.6e}", Re_L);
  Igor::Info("Reynolds number (gas)     = {:.6e}", Re_G);
  Igor::Info("Ohnesorge number          = {:.6e}", Oh);
