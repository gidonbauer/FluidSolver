...
auto main() -> int {
  ...
  if (!mdspan_to_npy(fs.xm, Igor::detail::format("{}/xm.npy", OUTPUT_DIR))) { return 1; }
  ...
  if (!mdspan_to_npy(fs.ym, Igor::detail::format("{}/ym.npy", OUTPUT_DIR))) { return 1; }
  // = Initialize grid =============================================================================

  // = Initialize flow field =======================================================================
  std::fill_n(fs.rho.get_data(), fs.rho.size(), 0.9);
  std::fill_n(fs.p.get_data(), fs.p.size(), 0.0);
  std::fill_n(fs.visc.get_data(), fs.visc.size(), 1e-3);

  for (size_t i = 0; i < fs.U.extent(0); ++i) {
    for (size_t j = 0; j < fs.U.extent(1); ++j) {
      // fs.U[i, j] = std::sin(fs.x[i] * 2 * std::numbers::pi);
      // fs.U[i, j] = std::sin(fs.y[j] * 2 * std::numbers::pi);
      fs.U[i, j] = 0.0;
    }
  }
  for (size_t i = 0; i < fs.V.extent(0); ++i) {
    for (size_t j = 0; j < fs.V.extent(1); ++j) {
      // fs.V[i, j] = std::sin(fs.x[i] * 2 * std::numbers::pi);
      // fs.V[i, j] = std::sin(fs.y[j] * 2 * std::numbers::pi);
      fs.V[i, j] = 0.0;
    }
  }
  ...
  while (t < T_END) {
    ...
    for (size_t sub_iter = 0; sub_iter < NUM_SUBITER; ++sub_iter) {
      ...
      for (size_t i = 0; i < fs.U.extent(0); ++i) {
        for (size_t j = 0; j < fs.U.extent(1); ++j) {
          fs.U[i, j] = fs.U_old[i, j] + dt * resU[i, j];
          fs.V[i, j] = fs.V_old[i, j] + dt * resV[i, j];
        }
      }
      ...
      calc_divergence(fs, div);
      calc_pressure_correction(fs, div, ps, dt, delta_p);
