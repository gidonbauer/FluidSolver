...
constexpr void calc_ib_correction_circle(const Circle<Float> wall,
                                         Float dx,
                                         Float dy,
                                         const Field1D<Float, NX, NGHOST>& xs,
                                         const Field1D<Float, NY, NGHOST>& ys,
  ...
  for_each_i<Exec::Parallel>(ib_corr, [&](Index i, Index j) {
    ...
    if ((flow & Flow::WALL_TO_RIGHT) > 0) {
      ...
      IGOR_ASSERT(
          0.0 < dist && dist < dx, "Expected dist in [0, {:.6e}] but got dist = {:.6e}", dx, dist);
    ...
    if ((flow & Flow::WALL_TO_TOP) > 0) {
      ...
      IGOR_ASSERT(
          0.0 < dist && dist < dy, "Expected dist in [0, {:.6e}] but got dist = {:.6e}", dy, dist);
