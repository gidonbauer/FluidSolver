...
constexpr auto eval_field_at_sf(const Field1D<Float, NX, NGHOST>& xm,
                                ...
                                Float x,
                                Float y) -> Float {
  constexpr Index N_NEIGHBOURS = 5 + 8;
  constexpr Index N_SHAPE_FUNC = 4;
  auto shape_funcs             = [](Float x, Float y) -> Vector<Float, N_SHAPE_FUNC> {
    return {1.0, x, y, x * y};
  };
  ...
  Matrix<Float, N_SHAPE_FUNC, N_SHAPE_FUNC> A{};
  Vector<Float, N_SHAPE_FUNC> b{};

  const auto [inn, jnn]                                = find_nearest_neighbor(xm, ym, x, y);
  const std::array<IndexPair, N_NEIGHBOURS> neighbours = {
      IndexPair{inn, jnn},
      IndexPair{inn + 1, jnn},
      IndexPair{inn - 1, jnn},
      IndexPair{inn, jnn + 1},
  ...
  for (auto [i, j] : neighbours) {
    ...
    const Vector<Float, N_SHAPE_FUNC> p = shape_funcs(pos.x, pos.y);
    const Float w                       = weight_function(pos, center, box_size);

    for (Index ii = 0; ii < N_SHAPE_FUNC; ++ii) {
      for (Index jj = 0; jj < N_SHAPE_FUNC; ++jj) {
        A(ii, jj) += w * p(ii) * p(jj);
      }
    }
    for (Index ii = 0; ii < N_SHAPE_FUNC; ++ii) {
      b(ii) += w * p(ii) * field(i, j);
    }
  }

  Vector<Float, N_SHAPE_FUNC> a{};
  solve_linear_system(A, b, a);
  Float res = 0.0;
  dot(a, shape_funcs(x, y), res);
  return res;
